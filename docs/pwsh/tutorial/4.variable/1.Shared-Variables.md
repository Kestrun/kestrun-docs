...
> **Note**: Variables defined this way are also discoverable via `Get-KrSharedState -Name <varname>`.
> The implicit injection mechanism and explicit `Set-KrSharedState` cmdlet both expose entries through
> the same unified state store.
...
Go back to [Response Caching][Previous] or continue to [Logging][Next].
...
[Previous]: ../3.static/6.Response-Caching
...
For explicit cmdlet-managed state and atomic operations, read the
[Shared State & Runspaces Guide][SharedStateGuide] and follow Tutorial 4.2.
