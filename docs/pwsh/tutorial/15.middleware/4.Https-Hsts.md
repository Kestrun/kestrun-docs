...
{% include examples/pwsh/15.4-Https-Hsts.ps1 %}

## Full source

File: [`pwsh/tutorial/examples/15.4-Https-Hsts.ps1`][15.4-Https-Hsts.ps1]
...
2. **Dual Endpoints**: Both HTTP (port 5000) and HTTPS (port 5001) listeners are configured
...
1. **Run the sample**:
   ...
   ```powershell
   .\15.4-Https-Hsts.ps1 -Port 8080
   ```

2. **Test HTTP to HTTPS redirect**:
   - Visit `http://localhost:8080`
   - Notice the redirect to `https://localhost:8081`

3. **Verify HSTS header**:

   ```powershell
   $response = Invoke-WebRequest -Uri "https://localhost:8081" -SkipCertificateCheck
...
[15.4-Https-Hsts.ps1]: /pwsh/tutorial/examples/15.4-Https-Hsts.ps1
...
Previous: [HTTPS Redirection](./3.Https-Redirection)
Next: [SignalR](./5.SignalR)
