...
| HTTPS Port | Inferred (443) | Specify when running on a custom dev port (e.g. 5001) |
...
File: [`pwsh/tutorial/examples/15.3-Https-Redirection.ps1`][15.3-Https-Redirection.ps1]

```powershell
{% include examples/pwsh/15.3-Https-Redirection.ps1 %}
```

## Step-by-step

1. Two endpoints are registered: an HTTP listener (e.g. `http://localhost:5000`) and an HTTPS listener (e.g. `https://localhost:5001`).
...
```powershell
Add-KrHttpsRedirection -RedirectStatusCode 308 -HttpsPort 5001
```
...
```powershell
pwsh .\docs\_includes\examples\pwsh\15.3-Https-Redirection.ps1 -Port 5000
```
...
| Wrong port in Location | Missing `-HttpsPort` when using a non-standard HTTPS port | Supply `-HttpsPort` (e.g. 5001) |
...
Previous: [Response Compression](./2.Compression)
Next: [HTTPS Strict Transport Security (HSTS)](./4.Https-Hsts)

[15.3-Https-Redirection.ps1]: /pwsh/tutorial/examples/15.3-Https-Redirection.ps1
