...
File: [`examples/PowerShell/Tutorial/2.5-Route-Group.ps1`][2.5-Route-Group.ps1]

```powershell
<#
    Sample Kestrun Server demonstrating Route Groups.
    Shows how to share a common prefix and still access request data
    (route, query, body, header, cookie) inside grouped routes.
    FileName: 2.5-Route-Group.ps1
#>
...
# Add the PowerShell runtime (required for PowerShell routes / middlewares)
...
Add-KrRouteGroup -Prefix "/input" -ScriptBlock {
    ...
    # Query string example (verb PATCH)
    ...
    # Body parameter example (verb POST)
    ...
    # Header parameter example (verb PUT)
    ...
    # Cookie parameter example (verb DELETE)
...
[Next]: ../3.static/index
[2.5-Route-Group.ps1]: https://github.com/Kestrun/Kestrun/blob/main/examples/PowerShell/Tutorial/2.5-Route-Group.ps1
