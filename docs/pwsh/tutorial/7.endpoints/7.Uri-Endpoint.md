...
If you do not pass an explicit target at all, `Add-KrEndpoint` can also resolve the listener from
`ASPNETCORE_URLS` or `PORT`. `-Uri` still takes precedence over both environment variables.
...
If you also specify `-PassThru`, the cmdlet returns endpoint spec strings derived from the resolved
binding. Those values can be reused with `Add-KrMapRoute -Endpoints` for listener-specific routing.

Environment precedence reminder:
...
```powershell
$endpointSpecs = Add-KrEndpoint -Uri ([uri]::new('http://localhost:5005')) -PassThru

Add-KrMapRoute -Pattern '/hello' -Verbs Get -Endpoints $endpointSpecs -ScriptBlock {
    Write-KrTextResponse 'Hello from URI-bound listener'
}
```

Using `-PassThru` avoids duplicating the `host:port` string in route definitions.
