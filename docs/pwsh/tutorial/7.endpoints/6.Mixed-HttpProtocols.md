...
> HTTP/3 requires OS + .NET support (QUIC). On Linux, install `libmsquic` before using an `Http3` endpoint.

## Linux requirement (HTTP/3)

On Ubuntu/Debian runners or machines, install the QUIC runtime library before starting an endpoint configured with `-Protocols Http3`:
...
| Symptom                   | Cause                | Fix                                      |
| ------------------------- | -------------------- | ---------------------------------------- |
| HTTP/3 listener fails     | QUIC not supported or `libmsquic` missing on Linux | Install `libmsquic` (Linux), or remove HTTP/3 endpoint |
