...
## Auto Client-Error Responses (Contract Enforcement)

This tutorial configures auto-generated OpenAPI client-error responses with:
...
- Parameter/body parsing failures can surface as **400**.
- Validation failures can surface as **422**.
- Request content-type mismatches can surface as **415**.
- Response negotiation mismatches can surface as **406**.

Kestrun can emit these 4xx responses in OpenAPI so the contract matches runtime behavior.
Use `Set-KrOpenApiErrorSchema` when you want a predictable schema name/media types across docs, tests, and client generation.
