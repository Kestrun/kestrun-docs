...
8. Create reusable response components as variables (`$OrderResponseDefault`, `$ErrorResponseDefault`).
9. POST endpoint: Accept CreateOrderRequestBody, validate, return OrderResponse or ErrorDetail.
10. GET endpoint: Retrieve order, validate UUID format, return OrderResponse or ErrorDetail.
11. PUT endpoint: Update order using CreateOrderRequestBody, return OrderResponse or ErrorDetail.
12. Build and test OpenAPI document.
...
├─ Response 201: OrderResponseDefault
└─ Response 400: ErrorResponseDefault

GET /orders/{orderId}
├─ Parameter: orderId (path, UUID format)
├─ Response 200: OrderResponseDefault
└─ Response 400: ErrorResponseDefault
...
You can attach OpenAPI vendor extensions to **requestBody** and **response** components using `[OpenApiExtension]`.

- Keys must start with `x-`.
- In the OpenAPI JSON, extensions appear under:
  - `components.requestBodies.<name>.x-*`
  - `components.responses.<name>.x-*`

In this tutorial, `CreateOrderRequestBody`, `OrderResponseDefault`, and `ErrorResponseDefault` include `x-kestrun-demo` extensions.

## Troubleshooting

**Issue**: Components not reused across endpoints.

- **Solution**: Ensure `[OpenApiRequestBodyRef]` and `[OpenApiResponseRefAttribute]` `ReferenceId` values match the declared component names.
...
Previous: [Response Components](./5.Response-Components)
Next: [Tags and External Docs](./7.Tags-and-Docs)
