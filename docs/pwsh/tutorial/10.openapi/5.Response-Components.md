...
- **Type on variable**: `[ErrorResponse]$NotFound = NoDefault` — schema inferred from variable type.
...
## Related: Contract-Enforced 4xx Responses

This page focuses on **explicit** reusable response components (`components.responses`).

For **auto-generated** client-error responses tied to runtime contract enforcement (400/406/415/422), see:

- [Parameter Components – Auto Client-Error Responses](./4.Parameter-Components#auto-client-error-responses-contract-enforcement)
- [Set-KrOpenApiErrorSchema][Set-KrOpenApiErrorSchema] to control schema name/content types for those autogenerated errors.
...
- In the OpenAPI JSON, extensions appear under `components.responses.<name>.x-*`.
...
Previous: [Parameter Components](./4.Parameter-Components)
Next: [Complete Components](./6.Complete-Components)
