...
| [WriteFormUrlEncodedResponse](KestrunResponse/WriteFormUrlEncodedResponse)(…) | Writes a form-urlencoded response with the specified input object, status code, and optional content type. Automatically converts the input object to a Dictionary{string, string} using [`ObjectToDictionaryConverter`](../Kestrun.Utilities/ObjectToDictionaryConverter). |
| [WriteFormUrlEncodedResponseAsync](KestrunResponse/WriteFormUrlEncodedResponseAsync)(…) | Asynchronously writes a form-urlencoded response with the specified input object, status code, and optional content type. Automatically converts the input object to a Dictionary{string, string} using [`ObjectToDictionaryConverter`](../Kestrun.Utilities/ObjectToDictionaryConverter). |
| [WriteHtmlResponse](KestrunResponse/WriteHtmlResponse)(…) | Writes an HTML response, rendering the provided template byte array and replacing placeholders with values from the given dictionary. (2 methods) |
| [WriteHtmlResponseAsync](KestrunResponse/WriteHtmlResponseAsync)(…) | Asynchronously writes an HTML response, rendering the provided template string and replacing placeholders with values from the given dictionary. (2 methods) |
...
* [KestrunResponse.cs](https://github.com/Kestrun/Kestrun/tree/main/src/CSharp/Kestrun/Models/KestrunResponse.cs)
